...
#reader(lib "htdp-beginner-abbr-reader.ss" "lang")((modname CS1101A21Exam2CanvasSample-2) (read-case-sensitive #t) (teachpacks ()) (htdp-settings #(#t constructor repeating-decimal #f #t none #f () #f)))
...
(check-expect (beastly-boats (list (make-boat "j" 1000 25) ))0)
(check-expect (beastly-boats (list (make-boat "j" 1000 25) ))0)
(check-expect (beastly-boats (list (make-boat "j" 1001 25) ))1)
(check-expect (beastly-boats (list (make-boat "j" 1001 26)(make-boat "j" 1001 26)(make-boat "j" 1001 26)(make-boat "j" 1000 26) (make-boat "j" 1001 25)))4)

 (define (beastly-boats alob)
   (cond [(empty? alob)  0 ]
         [(cons?  alob)  (if (beastly-boats--beastly? (first alob))
                             (+ 1    (beastly-boats  (rest alob)))
                                  (beastly-boats  (rest alob)))]))
;beastly-boats--beastly? Boat -> Boolean
; consumes a boat, produces true if beastly more than 1000 horsepower and have a capacity of at least 25
 (define (beastly-boats--beastly? a-boat)
   (and
         (> (boat-max-hp    a-boat) 1000)
...
;(list (first MASS) (first (rest (rest GLOBE))))

;(list WPI (first (rest MASS)))
...
(check-expect (insert-taxi ALLCABS 30 "Blui" 2022)
              (make-taxi 100 "Hildy" 2020
                          (make-taxi 50 "Travis" 2019  (make-taxi 30 "Blui" 2022 false false) false)
                          (make-taxi 150 "Bickle" 2018 false
                                    (make-taxi 200 "Max" 2019 false false))))
(check-expect (insert-taxi (make-taxi 20 "Blui" 2022 false false) 30 "Blui" 2022)
             (make-taxi 20 "Blui" 2022 false (make-taxi 30 "Blui" 2022 false false)))
(check-expect (insert-taxi (make-taxi 20 "Blui" 2022 false false) 30 "Blui" 2022)
             (make-taxi 20 "Blui" 2022 (make-taxi 10 "Blui" 2022 false false) false ))



;Template
#; (define (insert-taxi a-taxi nat name year)
    (cond [(boolean? a-taxi) (... );base case
          [(taxi?    a-taxi) (... (taxistuff ... )
                                  ( ... )
                                  ( ... a-taxi)
                                  (insert-taxi (taxi-left  a-taxi))
                                  (insert-taxi (taxi-right a-taxi)))]]))
