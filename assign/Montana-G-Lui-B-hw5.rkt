...
#reader(lib "htdp-intermediate-lambda-reader.ss" "lang")((modname Montana-G-Lui-B-hw5) (read-case-sensitive #t) (teachpacks ()) (htdp-settings #(#t write repeating-decimal #f #t none #f () #f)))
...
(define (list-acidic-rivers a-river)
  (if (<   (river-pH a-river) 6.5) ;Number
...
(define (unhealthy? a-river)
  (if 
   (or  (< (river-pH a-river) 6.5)
        (>  (river-pH a-river) 8.0)  ;Number
        (< (river-do a-river) 6))  ;Number
   true
...
(define (unhealthy?--lor a-lor)
  (cond
    [(empty? a-lor) false]
    [(cons? a-lor) (local [(define TRY (not (false?  (unhealthy? (first a-lor)))))]
                     (if TRY
                         TRY
...
(define (lower-all-ph a-river)
  (local [
          (define LOWER-BY 0.3)
          (define (lower-all-ph--river a-river)
            (make-river (river-name a-river) 
                        (- (river-pH a-river) LOWER-BY)  
                        (river-do a-river)  
                        (lower-all-ph--lor (river-tributaries a-river))))
          (define (lower-all-ph--lor a-lor) ;using map
            (map lower-all-ph--river a-lor))
          ]
    (lower-all-ph--river a-river)
    ))  

; lower-all-ph--lor: ListOfRiver -> ListOfRiver
; consumes a river, produces a ListOfRiver with all pH lowered by 0.3
(check-expect (lower-all-ph--lor-nomap empty) empty)
(check-expect (lower-all-ph--lor-nomap (list (make-river "Sup" 8.1 8 empty))) (list (make-river "Sup" 7.8 8 empty)))
(check-expect (lower-all-ph--lor-nomap (list (make-river "Sup" 8.3 8 empty))) (list (make-river "Sup" 8 8 empty)))

(define (lower-all-ph--lor-nomap a-lor) ; not using map - not used
  (cond
    [(empty? a-lor) empty]
    [(cons? a-lor) (cons 
                    (lower-all-ph (first a-lor)) 
                    (lower-all-ph--lor-nomap (rest a-lor)))]))
...
(define (find-subsystem rivname a-river)
  (if (string=? rivname (river-name a-river))
      a-river
      (find-subsystem--lor rivname (river-tributaries a-river))))

; find-subsystem--lor: ListOfRiver -> River or false
...
(define (all-same-kind? a-order kind)
  (local [
          (define (all-same-kind?--order a-order)
            (andmap all-same-kind?--menu-item? a-order))
          (define (all-same-kind?--menu-item? a-mi)
            (string=? (menu-item-kind a-mi) kind))
          ]
