...
#reader(lib "htdp-advanced-reader.ss" "lang")((modname cs1101A21Day26before-lambdastuff) (read-case-sensitive #t) (teachpacks ()) (htdp-settings #(#t constructor repeating-decimal #t #t none #f () #t)))
;#lang racket
...
(define TWO (map (lambda (x) (* 2 x) )TWENTY))
;;filter  (HOF)
...
; (map (lambda (x) (- x 10)) (build-list 10 identity))


;;apply: (X -> X) ListOfX -> X

(define FOONUMS (list 9 2 3 4 -5 4 3 2 1))
(define FOONUMS2 (list 1 2 3 4 5 6 7 8 9))
...
;(foldr string-append "" FOOSTRINGS)
...
(define PERF1 (filter perfect? (map add1 (build-list 10000 identity))))
(define PERF2 (filter perfect? (map (lambda (x) (* 2 x)) (map add1 (build-list 5000 identity)))))
