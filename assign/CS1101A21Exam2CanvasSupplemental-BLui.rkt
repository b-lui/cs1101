...
(check-expect (any-esperanto? empty) false)
(check-expect (any-esperanto? (list (make-tour "Allen" 1
                                               "DANG" "Esperanto"))) true)
(check-expect (any-esperanto? (list (make-tour "Allen" 10 "DANG" "English")
                                    (make-tour "Allen" 10 "DANG" "English")
                                    (make-tour "Allen" 10 "DANG" "English")
                                    (make-tour "Allen" 10 "DANG" "English"))) false)
(check-expect (any-esperanto? (list (make-tour "Allen" 10 "DANG" "English")
                                    (make-tour "Allen" 10 "DANG" "English")
                                    (make-tour "Allen" 10 "DANG" "English")
                                    (make-tour "Allen" 10 "DANG" "Esperanto"))) true)

(define (any-esperanto? alot)
  (cond [(empty? alot)  false ]
        [(cons?  alot)  (or (any-esperanto?--tour? (first alot))
                            (any-esperanto?  (rest alot)))]))
;;any-esperanto?--tour?: Tour -> Boolean
; consumes a Tour, produces true if is led by a guide whose language is Esperanto
(check-expect (any-esperanto?--tour? (make-tour "Allen" 10 "DANG" "Esperanto")) true)
(check-expect (any-esperanto?--tour? (make-tour "Allen" 10 "DANG" "lol")) false)

(define (any-esperanto?--tour? a-tour)
  (string=?  
   (tour-language  a-tour) "Esperanto"))
...
(list (first BARNYARD) (first (rest BARNYARD)) MASCOT)
...
(check-expect (update-model-year  (make-taxi 55 "b" 2020 (make-taxi 15 "b" 2020 false false) false) 15 2021)
              (make-taxi 55 "b" 2020 (make-taxi 15 "b" 2021 false false) false))
(check-expect (update-model-year  (make-taxi 55 "b" 2020  false (make-taxi 65 "b" 2020 false false)) 65 1995)
              (make-taxi 55 "b" 2020  false (make-taxi 65 "b" 1995 false false)))
(check-expect (update-model-year  ALLCABS 200 2000)
              (make-taxi 100 "Hildy" 1944
                         (make-taxi 50 "Travis" 1976 false false)
                         (make-taxi 150 "Korben" 2263 
                                    (make-taxi 135 "Reverend Jim" 1978 false false)
                                    (make-taxi 200 "Max" 2000 false false))))
(check-expect (update-model-year  (make-taxi 55 "b" 2020  false (make-taxi 65 "b" 2020 false false)) 10 2000)
              (make-taxi 55 "b" 2020  false (make-taxi 65 "b" 2020 false false))) ;not updated /found

(define (update-model-year a-taxi medallian newYear)
  (cond [(boolean? a-taxi) false];base case
        [(taxi?    a-taxi) (cond [(< medallian (taxi-medallion a-taxi) )
                                  (make-taxi (taxi-medallion a-taxi)
                                             (taxi-driver a-taxi)
                                             (taxi-year  a-taxi)
                                             (update-model-year (taxi-left  a-taxi) medallian newYear)
                                             (taxi-right a-taxi))]
                                 [(> medallian (taxi-medallion a-taxi) )
                                  (make-taxi (taxi-medallion a-taxi)
                                             ...
                                             (taxi-year  a-taxi)
                                             (taxi-left  a-taxi)
                                             (update-model-year (taxi-right a-taxi) medallian newYear))]
                                 [(= medallian (taxi-medallion a-taxi) )
                                  (make-taxi (taxi-medallion a-taxi)
                                             (taxi-driver a-taxi)
                                             newYear
...
(check-expect (kount-files O1) 10)
(check-expect (kount-files CS1101) 100)
(check-expect (kount-files ALL-TERMS) 780)
(check-expect (kount-files--lof empty) 0)
(check-expect (kount-files--lof (list A B C D)) 660)

(define (kount-files a-folder)
  (+
   (folder-files  a-folder)
   (kount-files--lof (folder-subfolders a-folder))))

; kount-files--lof:  ListOfFolder -> Natural
;;consumes a list of folder and returns the total number of
;;ordinary files contained in all the folders
(define (kount-files--lof alof)
  (cond [(empty? alof) 0]
        [(cons? alof)  (+ (kount-files (first alof))
                          (kount-files--lof (rest alof)))]))
