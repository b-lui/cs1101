...
(define (short-strings limit alos)
  (local [(define (short-strings--los alos)
            (filter short-strings--string alos))
          (define (short-strings--string a-string)
            (< (string-length a-string) limit))]
    (short-strings--los alos)))
...
(check-expect (big-boats empty) empty)
(check-expect (big-boats LIST2) LIST2)
(check-expect (big-boats LISTALLBOATS)
              (list
               (make-boat "Margaret Todd" 10 300)
               (make-boat "Cirle Line" 800 2000)
               (make-boat "Titanic" 25 1000)
               (make-boat "The Beast" 1400 100)
               (make-boat "Barely Beastly" 1001 25)))
...
(define (big-boats a-lob)
  (local [
          (define (big-boats--lob a-lob)
            (filter big-boat--boat? a-lob))
          (define (big-boat--boat? a-boat)
            (>= (boat-capacity a-boat) 25))
          ]
    (big-boats--lob a-lob)))
...
(define (boats-of-size size a-lob)
  (local [ (define (boats-of-size--lob a-lob)
             (filter boat-of-size--boat? a-lob))
           (define (boat-of-size--boat? a-boat)
             (>= (boat-capacity a-boat) size))]
    (boats-of-size--lob a-lob)))

(check-expect (boats-of-size 0 empty) empty)
(check-expect (boats-of-size 1000000 LISTALLBOATS) empty)
(check-expect (boats-of-size 0 LISTALLBOATS) LISTALLBOATS)
(check-expect (boats-of-size 1001 LISTALLBOATS) (list CIRCLELINE))
