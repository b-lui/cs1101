...
(check-expect (eye-color? "hazel" SUSANTREE) true)
(check-expect (eye-color? "purple" SUSANTREE) false)
(check-expect (eye-color? "brown" SUSANTREE) true)
(check-expect (eye-color? "" SUSANTREE) false)
(check-expect (eye-color? "blue" SUSANTREE) true)
;(define (eye-color? coloe a-person) false);stub
...
(check-expect (eye-color-list? "" empty) false)
(check-expect (eye-color-list? "blue" empty) false)
(check-expect (eye-color-list? "" (list (make-person "Joe" 1938 "green" empty)
                                        (make-person "Helen" 1940 "hazel"
                                                     (list (make-person "Beth" 1965 "green" empty)
                                                           (make-person "Sam" 1969 "brown" empty)))
                                        (make-person "Ricky" 1942 "blue" empty)))
              false)
(check-expect (eye-color-list? "brown" (list (make-person "Joe" 1938 "green" empty)
                                             (make-person "Helen" 1940 "hazel"
                                                          (list (make-person "Beth" 1965 "green" empty)
                                                                (make-person "Sam" 1969 "brown" empty)))
                                             (make-person "Ricky" 1942 "blue" empty)))
              true)
(check-expect (eye-color-list? "green" (list (make-person "Joe" 1938 "green" empty)
                                             (make-person "Helen" 1940 "hazel"
                                                          (list (make-person "Beth" 1965 "green" empty)
                                                                (make-person "Sam" 1969 "brown" empty)))
                                             (make-person "Ricky" 1942 "blue" empty)))
              true)
;(define (eye-color-list? color alop) false);stub
...
;; consumes an eye-color and a person (tree) and produces the name of the (first)
;;person in the tree with the given eye-color
(check-expect (eye-color-name "" SUSANTREE) false)
(check-expect (eye-color-name "blue" SUSANTREE) "Susan")
(check-expect (eye-color-name "hazel" SUSANTREE) "Helen")
(check-expect (eye-color-name "brown" SUSANTREE) "Sam")
(check-expect (eye-color-name-list "" empty)
              false)
(check-expect (eye-color-name-list "" (list (make-person "Joe" 1938 "green" empty)
                                            (make-person "Helen" 1940 "hazel"
                                                         (list (make-person "Beth" 1965 "green" empty)
                                                               (make-person "Sam" 1969 "brown" empty)))
                                            (make-person "Ricky" 1942 "blue" empty)))
              false)
(check-expect (eye-color-name-list "green" (list (make-person "Joe" 1938 "green" empty)
                                                 (make-person "Helen" 1940 "hazel"
                                                              (list (make-person "Beth" 1965 "green" empty)
                                                                    (make-person "Sam" 1969 "brown" empty)))
                                                 (make-person "Ricky" 1942 "blue" empty)))
              "Joe")
(check-expect (eye-color-name-list "blue" (list (make-person "Joe" 1938 "green" empty)
                                                (make-person "Helen" 1940 "hazel"
                                                             (list (make-person "Beth" 1965 "green" empty)
                                                                   (make-person "Sam" 1969 "brown" empty)))
                                                (make-person "Ricky" 1942 "blue" empty)))
              "Ricky")
;(define (eye-color-name color a-person) "");stub
...
;; consumes an eye-color and a person (tree) and produces the name of the (first)
;;person in the tree with the given eye-color 
;(define (eye-color-name-list color a-lop) "");stub

(define (eye-color-name-list color alop)
  (cond [(empty? alop) false]
        [else  (if   (not (false? (eye-color-name color (first alop))))
                     (eye-color-name color (first alop))
                     (eye-color-name-list color (rest alop)))]))


(check-expect (eye-color-name-all "blue" SUSANTREE) (list "Susan" "Ricky"))
(define (eye-color-name-all color a-person)
  (if (string=? color
                (person-eye a-person))
      (cons (person-name a-person) (eye-color-name-all-list color (person-children a-person)))
      (eye-color-name-all-list color (person-children a-person)))) 


(define (eye-color-name-all-list color alop)
  (cond [(empty? alop) empty]
        [else  ;(if   (not (false? (eye-color-name-all color (first alop))))
                      (append (eye-color-name-all color (first alop))  (eye-color-name-all-list color (rest alop)))
                     ]))
